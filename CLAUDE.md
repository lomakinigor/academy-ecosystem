...
**Правило auto-compact (из глобального ~/.claude/CLAUDE.md, обязательно для всех приложений):**

- На **60–65% заполнения контекста** — немедленно выполнить `/compact`
- На **75%+** — рассмотреть `/clear` если текущая задача завершена
- В начале новой большой задачи — `/clear` если предыдущая закрыта

**Признаки засорённого контекста (Context Rot — деградация точности 15–47%):**

- Claude путается в недавних изменениях
- Повторяет уже отвергнутые подходы
- Игнорирует свежие инструкции
- Ссылается на устаревший код

При любом из этих признаков — `/compact` или `/clear` без раздумий.

Полные правила: `~/.claude/skills/context-economy/SKILL.md`
